...
public class TairProtocolDecoder extends CumulativeProtocolDecoder {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(TairProtocolDecoder.class);
	...
	protected boolean doDecode(IoSession session, ByteBuffer in, ProtocolDecoderOutput out)
        ...
        if (session.getHandler() instanceof TairClientProcessor) {
        	TairClientProcessor proccessor = (TairClientProcessor)session.getHandler();
        	proccessor.responseCaught(chid);
        }
